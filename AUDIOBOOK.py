...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.inputtxt = customtkinter.CTkEntry(master=self.frame_right, textvariable=self.inpvariable)
        self.inputtxt.grid(row=5,column=0,columnspan=2, pady=10, padx=20, sticky="we")
        ...
        self.imgbtn = PhotoImage(file='mic_icon.png')

        self.button_6 = customtkinter.CTkButton(master=self.frame_left,
                                                image=self.imgbtn,
                                                text=None,
                                                command=self.speechtotext)
        self.button_6.grid(row=5, column=0, pady=1, padx=2)

        self.button_7 = customtkinter.CTkButton(master=self.frame_right,
                                                text="Search Video",
                                                command=self.ytsearch)
        self.button_7.grid(row=6, column=1, pady=10, padx=20)

        global options,dropdown
        options = ['Select a book','The Adventures of Sherlock-Holmes','Don Quixote','Pride and Prejudice']
        self.dropdown = customtkinter.CTkOptionMenu(master=self.frame_right,
                                                    values=options,
                                                    command=self.choosebk)        
        self.dropdown.grid(row=6,column=0,pady=10, padx=20)
    ...
    def choosebk(self,selection):
        global book,pages,pdfreadder
        print(selection)
        book="D:/Projects/Minor Project Sem-5/audiobook v3/" + str(selection) + ".pdf"
    ...
    def speechtotext(self):
        ...
        with sr.Microphone() as source:
            ...
            speechtext = r.recognize_google(audio_data)
            print(speechtext)
            self.inputtxt.insert(0,speechtext)
            
    def ytsearch(self):
        search_var = self.inputtxt.get()
